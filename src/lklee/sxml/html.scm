...
(define (element->html tag attrs body port symbol-dict)
  "Write the HTML TAG to PORT, where TAG has the attributes in the
list ATTRS and the child nodes in BODY."
  (when (hash-ref symbol-dict tag)
    (if (null? attrs)
        (sxml->html (apply (hash-ref symbol-dict tag) body)
                    port symbol-dict)
        (sxml->html (apply (hash-ref symbol-dict tag) attrs body)
                    port symbol-dict)
      )
      )
  (unless (hash-ref symbol-dict tag)
      (format port "<~a" tag)
      ...
      (if (and (null? body) (void-element? tag))
          ...
          (begin
            (display #\> port)
            (for-each (cut sxml->html <> port symbol-dict) body)
            (format port "</~a>" tag))))
  )
...
(define* (sxml->html tree #:optional
  ...
  (match tree
    ...
    (((? symbol? tag) ('@ attrs ...) body ...)
     (element->html tag attrs body port symbol-dict))
    (((? symbol? tag) body ...)
     (element->html tag '() body port symbol-dict))
    ((nodes ...)
     (for-each (cut sxml->html <> port symbol-dict) nodes))
    ((? string? text)
...
(define* (sxml->html-string sxml
                           #:optional (symbol-dict (make-hash-table)))
  "Render SXML as an HTML string."
  (call-with-output-string
   (lambda (port)
     (sxml->html sxml port symbol-dict))))
