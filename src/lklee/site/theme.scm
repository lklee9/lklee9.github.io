...
(define (collection-template-pub site title posts prefix)
  ...
  (define pubs-norm (filter-pubs-by-status pubs "norm"))
  (define pubs-feat (filter-pubs-by-status pubs "feat"))
  `(,component:html-side-info
    (div (@ (id "content") (class "main-column"))
         (h2 "Featured Publications")
         ,@(if (equal? (length+ pubs-feat) 0) '()
               (cons* (html-list-items pubs-feat me)))
         ,@(if (equal? (length+ pubs-norm) 0) '()
               (cons* '(h2 "Other Publications")
                      (html-list-items pubs-norm me)
                      ))
         )))
